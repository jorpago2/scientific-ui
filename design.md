...
Application pages use one product header, workflow navigation, one controlled task panel, a dominant scientific stage, an optional contextual inspector and a concise status bar. On mobile, workflow navigation moves to the bottom, every icon shares the exact horizontal center axis of its visible label, and an open task panel becomes the active workspace layer; no mini-preview or minimap is introduced.
